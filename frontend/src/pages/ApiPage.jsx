...
const CATEGORIES = [
  ...
  },
  {
    ...
    title: 'Реактор',
    base: '/api/forum/',
    description: 'Вопросы и обсуждения в духе StackOverflow — посты с категориями и звёздным рейтингом, бесконечно вложенные комментарии, файлы-вложения.',
    endpoints: [
      {
        method: 'GET', path: '/categories/', title: 'Список категорий', auth: 'public',
        desc: 'Без пагинации — категорий мало, отдаются все сразу (используется как для фильтра в ленте, так и для выпадающего списка в форме создания поста).',
        response: `// 200 OK\n[\n  { "id": 3, "name": "Типизация", "slug": "tipizaciya", "description": "Gradual typing, auto, проверки типов.", "created_at": "2026-06-01T08:00:00Z" }\n]`,
      },
      {
        method: 'POST', path: '/categories/', title: 'Создать категорию', auth: 'admin',
        desc: 'created_by подставляется автоматически. slug генерируется из name (транслитерация + уникальный суффикс при коллизии, тот же механизм, что у новостей и постов). Раньше категории мог создавать любой senior+, теперь — строго role=admin (управление категориями убрано из Центра управления разработчиков, осталось только через API/Django admin).',
        body: [
          { name: 'name', type: 'string', required: true, desc: 'Должно быть уникальным.' },
          { name: 'description', type: 'string', desc: 'Необязательно.' },
        ],
        response: '// 201 Created — тот же формат, что элемент списка выше',
        errors: [
          { code: 400, desc: 'Категория с таким name уже существует.' },
          { code: 403, desc: 'Доступно только администраторам (role=admin) — senior-разработчиков без этой роли сюда больше не пускает.' },
        ],
      },
      {
        method: 'PATCH', path: '/categories/<id>/', title: 'Изменить категорию', auth: 'admin',
        body: [{ name: 'name', type: 'string' }, { name: 'description', type: 'string' }],
        response: '// 200 OK — обновлённая категория',
        errors: [{ code: 403, desc: 'Доступно только администраторам (role=admin).' }],
      },
      {
        method: 'DELETE', path: '/categories/<id>/', title: 'Удалить категорию', auth: 'admin',
        desc: 'У постов этой категории category становится null (on_delete=SET_NULL) — посты не удаляются.',
        response: '// 204 No Content',
        errors: [{ code: 403, desc: 'Доступно только администраторам (role=admin).' }],
      },
      {
        method: 'GET', path: '/topics/', title: 'Лента постов', auth: 'public',
        desc: 'Ответ постраничный (DRF PageNumberPagination, как у новостей) — count/next/previous/results с лёгким TopicListSerializer (без blocks и вложений). Скрытые посты (is_hidden) других пользователей в выдачу не попадают.',
        params: [
          { name: 'search', in: 'query', type: 'string', desc: 'Ищет по title, summary, body и имени автора одновременно.' },
          { name: 'tags', in: 'query', type: 'string', desc: 'Через запятую, до 5 — подходит пост хотя бы с одним из тегов (OR). tag= тоже работает.' },
          { name: 'category', in: 'query', type: 'string', desc: 'slug категории.' },
          { name: 'resolved', in: 'query', type: 'boolean', desc: '"true" — только посты с is_resolved=true, "false" — только нерешённые. Без параметра — без разницы.' },
          { name: 'sort', in: 'query', type: 'new | oldest | top_rated | most_discussed', desc: 'По умолчанию new (сортировка по дате создания, убывание). oldest — то же по возрастанию.' },
          { name: 'mine', in: 'query', type: 'boolean', desc: '"true" (для авторизованных) — только свои посты, включая скрытые.' },
          { name: 'page', in: 'query', type: 'integer' },
          { name: 'page_size', in: 'query', type: 'integer', desc: 'По умолчанию 20, максимум 100 — page_size=100 используется фронтом для подсчёта облака тегов.' },
        ],
        response: `// 200 OK\n{\n  "count": 57, "next": "http://.../api/forum/topics/?page=2", "previous": null,\n  "results": [\n    {\n      "id": 7, "slug": "kak-rabotaet-gradual-typing", "author": ${PUBLIC_USER.replace(/\n/g, '\n      ')},\n      "title": "Как работает gradual typing?", "summary": "Динамика vs строгие типы в ATOM.",\n      "category": { "id": 3, "name": "Типизация", "slug": "tipizaciya", "description": "...", "created_at": "..." },\n      "tags": ["typing"], "is_resolved": false, "is_hidden": false,\n      "avg_rating": 4.5, "ratings_count": 2, "comments_count": 3, "created_at": "2026-06-01T08:00:00Z"\n    }\n  ]\n}`,
      },
      {
        method: 'POST', path: '/topics/', title: 'Создать пост', auth: 'auth',
        desc: 'author подставляется автоматически. HTML внутри blocks[].text (приходит из Tiptap) санитизируется на бэкенде через bleach — допускаются только базовые теги форматирования, <script> и подобное вырезается. Не чаще одного поста в 6 часов на автора.',
        body: [
          { name: 'title', type: 'string', required: true },
          { name: 'summary', type: 'string', desc: 'Короткий тизер для карточки ленты («тема»), до 280 символов.' },
          { name: 'category_id', type: 'integer | null', desc: 'id категории, необязательно.' },
          { name: 'tags', type: 'array<string>' },
          { name: 'github_url', type: 'string', desc: 'Необязательная ссылка на репозиторий.' },
          { name: 'blocks', type: 'array<{text, image?}>', desc: 'До 10 блоков; text — HTML из Tiptap.' },
        ],
        response: '// 201 Created — формат TopicDetailSerializer, см. GET /topics/<slug>/ ниже',
        errors: [
          { code: 400, desc: 'Больше 10 блоков, либо category_id указывает на несуществующую категорию.' },
          { code: 429, desc: 'Уже публиковали пост менее 6 часов назад — текст ошибки содержит, сколько ждать.' },
        ],
      },
      {
        method: 'GET', path: '/topics/<slug>/', title: 'Открыть пост', auth: 'public',
        desc: 'Полный формат (TopicDetailSerializer) — включает body, blocks, attachments и my_rating (оценка текущего пользователя, если он авторизован и уже оценивал, иначе null). Скрытый пост виден по прямой ссылке только своему автору.',
        response: `// 200 OK\n{\n  "id": 7, "slug": "kak-rabotaet-gradual-typing", "author": ${PUBLIC_USER.replace(/\n/g, '\n  ')},\n  "title": "Как работает gradual typing?", "summary": "...", "body": "...",\n  "category": { "id": 3, "name": "Типизация", "slug": "tipizaciya", "description": "...", "created_at": "..." },\n  "tags": ["typing"], "blocks": [{ "text": "<p>...</p>", "image": "" }],\n  "github_url": "", "is_resolved": false, "is_hidden": false,\n  "avg_rating": 4.5, "ratings_count": 2, "my_rating": null,\n  "attachments": [{ "id": 9, "file": "https://atom.example/media/forum/7/a1b2.atm", "original_name": "main.atm", "size": 412, "created_at": "..." }],\n  "created_at": "2026-06-01T08:00:00Z"\n}`,
        errors: [{ code: 404, desc: 'Пост не найден, либо скрыт автором и запрашивает не он.' }],
      },
      {
        method: 'PATCH', path: '/topics/<slug>/', title: 'Изменить пост', auth: 'owner',
        desc: 'Изменить (включая is_hidden — скрыть/показать свой пост, и is_resolved — отметить вопрос решённым) может только автор. Скрытие не сбрасывает avg_rating/ratings_count — это намеренно, чтобы нельзя было «спрятать» пост с плохой оценкой и не потерять её в своём рейтинге.',
        body: [
          { name: 'title', type: 'string' }, { name: 'summary', type: 'string' },
          { name: 'category_id', type: 'integer | null' }, { name: 'tags', type: 'array<string>' },
          { name: 'github_url', type: 'string' }, { name: 'blocks', type: 'array' },
          { name: 'is_resolved', type: 'boolean' }, { name: 'is_hidden', type: 'boolean' },
        ],
        response: '// 200 OK — обновлённый пост',
        errors: [{ code: 403, desc: 'Это не ваш пост.' }],
      },
      {
        method: 'DELETE', path: '/topics/<slug>/', title: 'Удалить пост', auth: 'owner',
        desc: 'Каскадно удаляет комментарии, вложения и оценки этого поста.',
        response: '// 204 No Content',
        errors: [{ code: 403, desc: 'Это не ваш пост.' }],
      },
      {
        method: 'GET', path: '/topics/<slug>/comments/', title: 'Дерево комментариев', auth: 'public',
        desc: 'Все комментарии поста одним запросом — дерево собирается на бэкенде в Python (без N+1 запросов на каждый уровень вложенности) и отдаётся уже вложенным через replies; глубина не ограничена.',
        response: `// 200 OK\n[\n  {\n    "id": 15, "topic": 7, "author": ${PUBLIC_USER.replace(/\n/g, '\n    ')},\n    "parent": null, "body": "...",\n    "references": [\n      { "id": 1, "attachment": 9, "attachment_name": "main.atm", "start_line": 12, "end_line": 12 },\n      { "id": 2, "attachment": 9, "attachment_name": "main.atm", "start_line": 20, "end_line": 24 }\n    ],\n    "is_accepted_answer": false, "is_topic_author": false, "created_at": "...",\n    "replies": [\n      { "id": 16, "topic": 7, "author": { "...": "..." }, "parent": 15, "body": "...", "references": [], "is_accepted_answer": false, "is_topic_author": true, "created_at": "...", "replies": [] }\n    ]\n  }\n]`,
        notes: 'is_topic_author — вычисляемый флаг (author.id == topic.author.id), используется фронтом для бейджа «автор». Уровень/роль автора берётся из вложенного author.developer_level и author.is_developer. references — сколько угодно ссылок на диапазон строк файла на один комментарий (см. POST /comments/ ниже).',
      },
      {
        method: 'POST', path: '/topics/<slug>/rate/', title: 'Оценить пост звёздами', auth: 'auth',
        desc: 'Апсерт — повторный вызов с новым score просто меняет уже выставленную оценку, не создаёт вторую. avg_rating/ratings_count на посте пересчитываются сразу и возвращаются в ответе.',
        body: [{ name: 'score', type: 'integer 1-5', required: true }],
        response: `// 200 OK\n{ "avg_rating": 4.5, "ratings_count": 2 }`,
        errors: [{ code: 400, desc: 'score не передан или не входит в диапазон 1-5.' }],
      },
      {
        method: 'GET', path: '/comments/', title: 'Список комментариев', auth: 'public',
        desc: 'Возвращает ВСЕ комментарии всех постов без фильтра по посту — фронтенд практически всегда читает комментарии через GET /topics/<slug>/comments/ выше вместо этого эндпоинта.',
        response: `// 200 OK\n[\n  { "id": 15, "topic": 7, "author": ${PUBLIC_USER.replace(/\n/g, '\n  ')}, "parent": null, "body": "...", "references": [], "is_accepted_answer": false, "is_topic_author": false, "created_at": "...", "replies": [] }\n]`,
      },
      {
        method: 'POST', path: '/comments/', title: 'Добавить комментарий', auth: 'auth',
        desc: 'author подставляется автоматически. parent — id другого комментария, чтобы ответить именно на него (в том числе на свой собственный); без parent комментарий становится корневым.',
        body: [
          { name: 'topic', type: 'integer', required: true },
          { name: 'body', type: 'string', required: true },
          { name: 'parent', type: 'integer | null', desc: 'id комментария, на который отвечаем.' },
          { name: 'references', type: 'array<{attachment, start_line, end_line}>', desc: 'Сколько угодно ссылок на диапазон строк файла (до 10 на комментарий) — не одна, как раньше. attachment — id вложения из этого же поста, start_line/end_line — диапазон (равны друг другу для одной строки). Рендерится фронтом как чипы «файл:12» или «файл:20-24».' },
        ],
        response: '// 201 Created — тот же формат, что элемент списка выше (с пустым replies)',
        errors: [{ code: 400, desc: 'Ссылка ведёт на вложение из другого поста, end_line меньше start_line, или больше 10 ссылок в одном комментарии.' }],
      },
      {
        method: 'PATCH', path: '/comments/<id>/', title: 'Изменить комментарий', auth: 'owner',
        desc: 'Изменить (например, отредактировать текст, ссылки или снять/поставить is_accepted_answer) может только автор комментария. Если передать references — старые ссылки этого комментария полностью заменяются новым набором.',
        body: [{ name: 'body', type: 'string' }, { name: 'references', type: 'array<{attachment, start_line, end_line}>' }, { name: 'is_accepted_answer', type: 'boolean' }],
        response: '// 200 OK — обновлённый комментарий',
        errors: [{ code: 403, desc: 'Это не ваш комментарий.' }],
      },
      {
        method: 'DELETE', path: '/comments/<id>/', title: 'Удалить комментарий', auth: 'owner',
        desc: 'Каскадно удаляет все вложенные ответы (replies) этого комментария.',
        response: '// 204 No Content',
        errors: [{ code: 403, desc: 'Это не ваш комментарий.' }],
      },
      {
        method: 'POST', path: '/attachments/upload/', title: 'Приложить файл к посту', auth: 'owner',
        desc: 'Только текстовые/код-файлы (whitelist расширений: .atm, .txt, .py, .js, .cpp, .json и т.п.) — без архивов и бинарников, чтобы AttachmentsTerminal на фронте могло безопасно показать содержимое как текст. До 5 файлов на пост, каждый ≤200 КБ. Доступно только автору поста.',
        contentType: 'multipart/form-data',
        body: [
          { name: 'topic', type: 'integer', required: true, desc: 'id поста, к которому прикладывается файл.' },
          { name: 'file', type: 'file', required: true },
        ],
        response: `// 201 Created\n{ "id": 9, "file": "https://atom.example/media/forum/7/a1b2.atm", "original_name": "main.atm", "size": 412, "created_at": "..." }`,
        errors: [
          { code: 400, desc: 'Файл не передан, расширение не из whitelist, файл больше 200 КБ, либо уже 5 вложений на этом посте.' },
          { code: 403, desc: 'Прикладывать файлы может только автор поста.' },
        ],
      },
      {
        method: 'POST', path: '/upload-image/', title: 'Загрузить фото для блока поста', auth: 'auth',
        desc: 'Альтернатива вводу готового URL в блоке поста (PostBlockEditor.jsx) — вкладка «Загрузить файл». В отличие от такого же эндпоинта у новостей (/api/news/upload-image/) доступен любому авторизованному, не только мидл+ — постить в Реакторе может каждый.',
        contentType: 'multipart/form-data',
        body: [{ name: 'image', type: 'file', required: true, desc: 'PNG, JPEG, WebP или GIF, до 5 МБ.' }],
        response: '// 201 Created\n{ "url": "https://atom.example/media/forum/blocks/a1b2c3d4.png" }',
        errors: [{ code: 400, desc: 'Файл не передан, недопустимый тип или больше 5 МБ.' }],
      },
    ],
  },
  {
